...
class SmartCamera extends AppElements {
    constructor() {
        super();
    }
    ...
    App(source,event,portType,logger) {
        ...
        logger.infoLog.info(`LOGGER: SmartCamera's App element received incoming layout event from ${source} element on ${portType} port`);
        ...
        port.GetTapStatePort.call()
            .then(function (stateEvent) {
                ...
                logger.infoLog.info("LOGGER: SmartCamera got current Tap state: " + tapState);
            ...
            .then(function (responseEvent) {
                if (responseEvent) { // if we received a response event then the tap was opened and we successfully posted the image
                    logger.infoLog.info("SmartCamera App received this from the Tap/HttpRequest modules: " + JSON.stringify(responseEvent[0]));
                    logger.infoLog.info("SmartCamera has successfully send the frame to the remote storage")
                }
